...
import { MOCK_STUDENTS, MOCK_COLLEGES, MOCK_PHASE1_ALLOTMENTS, MOCK_PHASE2_ALLOTMENTS, MOCK_SEAT_AVAILABILITY, MOCK_OFFICIAL_CUTOFFS } from '../data/mockDataset';
...
const DEFAULT_DATA_SOURCES: DataSourceConfig[] = [
  {
    ...
    previewRows: [
      { htno: '2601011245', stud_name: 'Prasanth Kumar', rank: 1245, gen: 'Male', cat: 'OC', coll_code: 'ABCE', br_code: 'MEC', is_allotted: true },
      { htno: '2601012356', stud_name: 'Praveen Reddy', rank: 2356, gen: 'Male', cat: 'OC', coll_code: 'ABCE', br_code: 'MEC', is_allotted: true },
      { htno: '2601013489', stud_name: 'Pradeep Kumar', rank: 3489, gen: 'Male', cat: 'BC-D', coll_code: '', br_code: '', is_allotted: false },
    ],
  },
  {
    id: 'ds_phase2',
    ...
    previewRows: [
      { htno: '2601011245', stud_name: 'Prasanth Kumar', rank: 1245, gen: 'Male', cat: 'OC', coll_code: 'XYZT', br_code: 'CSE', is_allotted: true },
      { htno: '2601012356', stud_name: 'Praveen Reddy', rank: 2356, gen: 'Male', cat: 'OC', coll_code: 'ABCE', br_code: 'CSE', is_allotted: true },
      { htno: '2601013489', stud_name: 'Pradeep Kumar', rank: 3489, gen: 'Male', cat: 'BC-D', coll_code: 'ABCE', br_code: 'CSE', is_allotted: true },
    ],
  },
  {
    id: 'ds_seats',
    ...
    previewRows: [
      { inst_code: 'ABCE', branch: 'CSE', category: 'OC', gender: 'BOYS', vacant_seats: 2, total_intake: 60, counseling_phase: 'PHASE_2' },
      { inst_code: 'ABCE', branch: 'CSE', category: 'OC', gender: 'GIRLS', vacant_seats: 1, total_intake: 30, counseling_phase: 'PHASE_2' },
      { inst_code: 'ABCE', branch: 'CSE', category: 'EWS', gender: 'BOYS', vacant_seats: 1, total_intake: 12, counseling_phase: 'PHASE_2' },
    ],
  },
  {
    id: 'ds_cutoffs',
...
class DataRepository {
  ...
  private phase1Allotments: Record<string, PhaseAllotment> = { ...MOCK_PHASE1_ALLOTMENTS };
  private phase2Allotments: Record<string, PhaseAllotment> = { ...MOCK_PHASE2_ALLOTMENTS };
  private seatAvailability: SeatAvailability[] = [...MOCK_SEAT_AVAILABILITY];
  private officialCutoffs: CutoffRecord[] = [...MOCK_OFFICIAL_CUTOFFS];
  ...
  public getAnalyticsSummary() {
    ...
    const totalStudents = derived.length;
    
    let retainedCount = 0;
    let newSeatCount = 0;
    let transferredCount = 0;
    let upgradedBranchCount = 0;
    let noSeatCount = 0;

    for (const d of derived) {
      if (d.status === 'RETAINED') retainedCount++;
      else if (d.status === 'NEW_SEAT') newSeatCount++;
      else if (d.status === 'TRANSFERRED') transferredCount++;
      else if (d.status === 'UPGRADED_BRANCH') upgradedBranchCount++;
      else if (d.status === 'NO_SEAT') noSeatCount++;
    }

    const totalAllotted = totalStudents - noSeatCount;

    // College distribution
    const collegeDist: Record<string, { name: string; count: number }> = {};
    for (const d of derived) {
      if (d.finalCollegeCode && d.finalCollege) {
        if (!collegeDist[d.finalCollegeCode]) {
          collegeDist[d.finalCollegeCode] = { name: d.finalCollege, count: 0 };
        }
        collegeDist[d.finalCollegeCode].count++;
      }
    }
